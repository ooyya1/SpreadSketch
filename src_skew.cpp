...
int main(int argc, char* argv[]) {
...
#else
    if (argc != 3) {
        std::cout << "[Usage] " << argv[0] << "\t [Memory(MB)]" << "\t [zipf parameter]" << std::endl;
        return -1;
    }
#endif
    ...
    int c = 3;
    int memory = 438;
    cmwidth = allspace/(memory+lgn+8)/cmdepth;
    std::cout << "Memory: " <<  allspace << " width: " << cmwidth << std::endl;
    ...
    for (std::string file; getline(tracefiles, file);) {
        ...
        tuple_t t;
        adaptor->Reset();
        unsigned long src_sum = 0;
        unsigned long edge_sum = 0;
        while(adaptor->GetNext(&t) == 1) {
            edge_tp ip;
            ip.src_ip = t.src_ip; ip.dst_ip = t.dst_ip;
            sum.insert(ip);
            ground1[t.src_ip].insert(t.dst_ip);
        }
        edge_sum = sum.size();
        src_sum = ground1.size();
        ...
        std::vector<std::pair<key_tp, unsigned> > groundvec;
        groundvec.clear();
        for (auto it = ground1.begin(); it != ground1.end(); it++) {
            std::pair<key_tp, unsigned> p;
            p = std::make_pair(it->first, it->second.size());
            groundvec.push_back(p);
        }
        std::sort(groundvec.begin(), groundvec.end(), [=](std::pair<key_tp, unsigned>&a, std::pair<key_tp, unsigned>&b)
                {
        ...
        unsigned test = 0;
        for (i = 0; i < N; i++) {
            unsigned value = freq[i+1]*edge_sum;
        ...
        std::cout << "test=" << test << "\t total_pkt=" << edge_sum << "\t threshold=" << threshold << std::endl;
        free(freq);
        std::cout << "2.2 Finish reset pcap frequency according to the zipf sequence" << std::endl;

        //2.3 construct the skewed pacp trace
        unsigned long *buffer = (unsigned long*)malloc(edge_sum*sizeof(unsigned long));
        std::set<unsigned long> newedge;
        newedge.clear();
        int *shuffl = (int*)malloc(edge_sum*sizeof(int));
        for (i = 0; i < edge_sum; i++) {
            shuffl[i] = i;
        }

        //shufl
        for (i = edge_sum-1; i > 0; i--) {
            int j = rand() % (i+1);
        ...
        for (i = 0; i < N; i++) {
            for (int j = 0; j < groundvec[i].second; j++) {
                unsigned long tmp = groundvec[i].first;
                tmp = (tmp << 32) | rand();
                buffer[shuffl[index]] = tmp;
        ...
        t2 = now_us();
        free(buffer);
        throughput = edge_sum*1000000.0/(t2-t1);
        ...
        std::cout << std::setw(20) << std::left << "SS"
